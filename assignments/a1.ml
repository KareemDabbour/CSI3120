...
let fullname : string = name ^ " Dabbour"

(* 1c. Create a string containing your email address *)
let email : string = "kdabb095@uottawa.ca"
...
(* 1e. Replace the .... with one thing you hope to learn about in
   this course *)
let learning : string = "I hope to learn and improve my knowledge of certain programming language concepts"
...
let exp3a : string = "x was of type int while y was of type float which lead to a type mismatch in the sum. 
                     '*' is used to multiply two ints however due to y being a float it would not compile.
                     There are many ways to fix this issue, you could type-cast y to int for example. 
                     However, I decided to change x from int to float and use float-based operations."
...
let exp3b1 : string = "In the line where the recursion is being called, there weren't any parenhesis around the k-1 which lead ocaml to assume x is a function
and attempts to pass k-1 into x as a parameter. However, since x is of type int it can not be called as a function. "


(* Problem 4 *)
(* Consider the following incomplete program *)

(*Trivial function to allow for prob4 to compile *)
let f (a:string) (b:float) : bool = int_of_float(b) = String.length(a)
...
let exp4 : string = "The issue with prob4 is that there isnt a base case since it's calling itself without 
                     altering any of the parameters which leads to it calling itself an infinite amount of times."
...
let rev_string (str: string): string = 
   let rec helper (ind: int) :string = 
      if ind = String.length(str) then "" else helper(ind +1) ^ (String.make 1 str.[ind])
   in 
   helper 0
